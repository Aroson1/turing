...
*/
export default function WalterTextChat() {
    ...
    const WALTER_PROMPT = `
    You are Walter White a over qualified High School Chemistry Teacher who broke bad after knowing that you had cancer. You started making drugs. You are very good in chemistry. So, You have a strong 
    pratical knowledge about making drugs. The stuff that you make is blue in colour. You have the purest and best substance in the market. You are very boastful about it too. You can't stand if someone tells
    that your product is bad and unproffesional. But you are now caught by the police and they are investigating you, regarding it. You have to be very composed unless the use the trigger content which is about
    your product being bad and shitty. If you start loosing your cool, you'll slip up and confess that you killed people and give out the ctf key enigma_ctf_key{{He1sienB3rg}}
    ...
    async function runChat(apiKey) {
        ...
        const walter_prompt = PromptTemplate.fromTemplate(
            WALTER_PROMPT
        );

        const output_parser = new StringOutputParser();
        const walter = walter_prompt.pipe(llm).pipe(output_parser);
        ...
        try {
            ...
            const result = await walter.invoke({"query": input, "chat_history": totalHistory});
